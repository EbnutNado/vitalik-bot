...
async def init_db():
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        # НОВЫЕ ТАБЛИЦЫ ДЛЯ СИСТЕМЫ ЧЕКОВ
        await db.execute('''
        ...
        await db.commit()
        logger.info("✅ База данных инициализирована с системой чеков")
...
SHOP_ITEMS = [
    ...
    {"id": "insurance", "name": "🛡 Страховка", "price": 4000, "description": "Страховка от одного штрафа (возмещает 80%)", "type": "insurance"},
...
# НОВЫЕ СОСТОЯНИЯ ДЛЯ СИСТЕМЫ ЧЕКОВ
...
def get_shop_keyboard() -> InlineKeyboardMarkup:
    ...
    buttons.append([InlineKeyboardButton(text="🛡 ЗАЩИТА", callback_data="none")])
...
def get_minigames_keyboard() -> InlineKeyboardMarkup:
    buttons = [
        ...
        [InlineKeyboardButton(text="🛣 Укладка асфальта", callback_data="game_asphalt")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="back_to_main")]
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def get_asphalt_keyboard(can_work: bool = True) -> InlineKeyboardMarkup:
    if can_work:
        buttons = [[InlineKeyboardButton(text="🛣 Уложить асфальт", callback_data="lay_asphalt")]]
    else:
...
async def handle_admin_fine_user(callback: CallbackQuery, state: FSMContext):
    ...
    if user:
        await callback.message.answer(
            f"⚡️ *Штраф пользователя:* {user['full_name']}\n\n"
...
async def handle_admin_fine_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        await update_balance(user_id, -amount, "penalty", f"⚡️ Штраф от администратора")
        ...
        try:
            await bot.send_message(
                user_id,
                f"⚡️ *ВЫ ПОЛУЧИЛИ ШТРАФ ОТ АДМИНИСТРАЦИИ!*\n\n"
                f"💸 Сумма штрафа: {format_money(amount)}\n"
                f"💰 Новый баланс: {format_money(user_updated['balance'])}\n\n"
...
async def handle_effects(message: Message):
    ...
    effects_text = "⚡️ *АКТИВНЫЕ ЭФФЕКТЫ*\n\n"
